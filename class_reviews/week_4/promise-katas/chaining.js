...
describe('chaining multiple promises can enhance readability', () => {
  ...
  describe('chain promises', function() {
    ...
    //   const wordsPromise = Promise.resolve('Sentence without       an end ');
    //   return wordsPromise // 'Sentence without       an end '
    //     .then(appendPeriod) // 'Sentence without       an end .'
    //     .then(trim) // 'Sentence without       an end .'
    //     .then(removeMultipleSpaces) // 'Sentence without an end .'
    //     .then(actual => {assert.equal(actual, 'Sentence without an end.')})
    //   ;
    // });
    it('order of the `then()`s matters', function() {
    ...
    it('any of the things given to `then()` can resolve asynchronously (the real power of Promises)', function() {
      const wordsPromise = Promise.resolve('sentence without an end');
      return wordsPromise
        .then(s => new Promise(resolve => asyncUpperCaseStart(s, resolve))) // 'Sentence without an end'
        .then(s => new Promise(resolve => setTimeout(() => resolve(appendPeriod(s)), 100))) // 'Sentence without an end.'
        .then(actual => {assert.equal(actual, 'Sentence without an end.')})
      ;
    });
    it('also asynchronously, the order still matters, promises wait, but don`t block', function() {
      ...
      return wordsPromise // 'trailing space   '
        .then(s => new Promise(resolve => asyncUpperCaseStart(s, resolve))) // 'Trailing space   '
        .then(s => new Promise(resolve => setTimeout(() => resolve(trim(s)), 100))) // 'Trailing space'
        .then(s => new Promise(resolve => setTimeout(() => resolve(appendPeriod(s)), 100))) // 'Trailing space.'
